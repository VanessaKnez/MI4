...
function addBook(inline)
{
  ...
  if (inline) {
    ...
    var staat = $$('select[name=inptStateBoekToevoegen]').val();
    var school = $$('#inptSchoolBoekToevoegen').val();
    var verkoopOFVerhuur = $$('select[name=inptSellOrRentBoekToevoegen]').val();
    var prijs = $$('#inptPriceBoekToevoegen').val();
    let e = $('#submitButton').get(0).files[0];
    console.log("isbn:" + ISBN);
    console.log("titel:" + titel);
    console.log("auteur:" + auteur);
    console.log("staat:" + staat);
    console.log("school:" + school);
    console.log("verkoopofverhuur:" + verkoopOFVerhuur);
...
function laadPersoonlijkeBoeken()
{
  ...
  var peroonlijkeID = result_id + "persoonlijk";
  var result_ISBN = result.ISBN;
  var persoonlijkeISBN = result_ISBN + "persoonlijk";
  var result_prijs = result.prijs;
  var result_school = result.school;
    ...
    if(result_verhuur == "yes")
    {
      div.innerHTML += "\
            <div class='card card-expandable'>\
              ...
              <p>\
              ISBN: "+result_ISBN + "<br>\
              titel: " +result_titel + "<br>\
              auteur: " +result_auteur +" <br>\
              prijs: " +result_prijs + "<br>\
              staat: " +result_staat + "<br>\
              school: " +result_school + "<br>\
              You are able to rent this book.\
              <br>\
              <label>Add a comment:</label>\
              <textarea id="+peroonlijkeID +"></textarea>\
              <button class='col button button-outline button-round' onclick='setTimeout(commentaarToevoegenPersoonlijk(`" +peroonlijkeID+"`,`" +result_id+"`),10)'>Add comment</button>\
              <br>\
              ...
              <br>\
              <button class='col button button-outline button-round' onclick='setTimeout(toonCommentaarPersoonlijk(`" +result_id+"`, `" +persoonlijkeISBN+"`),10)'>Show comments</button>\
              </p>\
              <div id=" +persoonlijkeISBN+">\
              </div>\
              </div>\
                </div>\
              </div>";
  ...
  playersRef.on("value", function(snapshot) 
  {
    snapshot.forEach(function(childSnapshot) 
    {
      ...
      query.once("value", function(data) 
      {
        ...
        var result_ISBN = result.ISBN;
        var favorietenISBN = result_ISBN + "favorieten";
        var favorietenID = result_id + "favorieten";
        var result_prijs = result.prijs;
        var result_auteur = result.auteur;
        var result_staat = result.staat;
        var result_titel = result.titel;
        ...
        tangRef.getDownloadURL().then(function(url)
        {
          ...
          if(result_verhuur == "yes")
          {
            ...
            <div class='card card-expandable'>\
              ...
              <br>\
              <label>Add a comment:</label>\
              <textarea id="+favorietenID +"></textarea>\
              <button class='col button button-outline button-round' id = "+result_id+" onclick='verwijderPersoonlijkeFavoriet(`"+result_id+"`, `"+result_titel+"`)'>Remove from favorites</button><br>\
              <button class='col button button-outline button-round' onclick='setTimeout(commentaarToevoegenFavorieten(`" +favorietenID+"`,`" +result_id+"`),10)'>Add comment</button>\
              <br>\
              <button class='col button button-outline button-round' onclick='setTimeout(toonCommentaarFavorieten(`" +result_id+"`, `" +favorietenISBN+"`),10)'>Show comments</button>\
              </p>\
              <div id=" +favorietenISBN+">\
              </div>\
              </div>\
                </div>\
              </div>";
...
function commentaarToevoegen(x)
{
  ...
  var backupcomment = commentaar;
  commentaar = commentaar.replace(/\n/g, "");
  if(commentaar == "")
  {
    app.dialog.alert("Please fill in a comment", "Error");
  }
  else
  {
    commentaar = backupcomment;
    commentaar = commentaar.replace(/\r?\n/g, '<br/>');
    var today = new Date();
    ...
    userRef.set({
                ...
                app.dialog.alert("Your comment has been added", "Comment");
                

  }
  
}
function commentaarToevoegenFavorieten(x,y)
{
  console.log(x);
  var favorietenTextArea= x;
  var boekID = y;
  var commentaar = $$('#'+favorietenTextArea).val();
  console.log(commentaar);
  var backupcomment = commentaar;
  commentaar = commentaar.replace(/\n/g, "");
  if(commentaar == "")
  ...
  else
  {
    ...
    var today = new Date();
    var timestamp = today.getDate()+'-'+(today.getMonth()+1)+'-'+today.getFullYear() + " "+ today.getHours() + ":" + today.getMinutes();
    document.getElementById(favorietenTextArea).value = "";
    var chars = "0123456789ABCDEFGHIJKLMNOPQRSTUVWXTZabcdefghiklmnopqrstuvwxyz";
    var string_length = 20;
    var chatkey = '';
    for (var i=0; i<string_length; i++) 
...
function commentaarToevoegenPersoonlijk(x,y)
{
  console.log(x);
  var persoonlijkTextArea= x;
  var boekID = y;
  var commentaar = $$('#'+persoonlijkTextArea).val();
  console.log(commentaar);
  var backupcomment = commentaar;
  commentaar = commentaar.replace(/\n/g, "");
  if(commentaar == "")
  ...
  else
  {
    ...
    var today = new Date();
    var timestamp = today.getDate()+'-'+(today.getMonth()+1)+'-'+today.getFullYear() + " "+ today.getHours() + ":" + today.getMinutes();
    document.getElementById(persoonlijkTextArea).value = "";
    var chars = "0123456789ABCDEFGHIJKLMNOPQRSTUVWXTZabcdefghiklmnopqrstuvwxyz";
    var string_length = 20;
    var chatkey = '';
    for (var i=0; i<string_length; i++) 
...
function toonCommentaarFavorieten(x,y)
{
  var boekID = x;
  var ISBN = y;
  console.log(boekID);
...
function toonCommentaarPersoonlijk(x,y)
{
  var boekID = x;
  var ISBN = y;
  console.log(boekID);
...
{ 
  ...
  .then(function() 
  {
    app.dialog.alert("Book: " + titel + " has sucesfully been removed from favorites.", "Success");
    app.views.main.router.navigate("/")
    //location.reload();
  })
  .catch(function(error) 
...
function verwijderComments(x)
{
  var boekID = x;
  let userRef = firebase.database().ref('commentaar/' + boekID);
  userRef.remove();
}
function reloadPage()
{
